...
import { mapLiability, type DbLiabilityJoinedRow } from "../mappers/liabilities.mapper";
...
export class LiabilitiesRepo {
  ...
  private joinedSelect = `
    SELECT
      ...
      l.liability_type,
      l.name, l.apr, l.currency_code, l.current_balance,
  ...
  upsert(input: Liability): Liability {
    ...
    const insertBase = this.db.prepare(`
      INSERT INTO liability (
        ...
        liability_type, name, apr, currency_code, current_balance,
    ...
    const tx = this.db.transaction(() => {
      ...
      if (!existing) {
        const r = insertBase.run(
          ...
          input.apr ?? null,
          input.currencyCode,
          input.currentBalance as any as number,
        ...
        // Type change => remove old subtype row first
      ...
      // Subtype upsert (ensure row exists even if fields are null)
